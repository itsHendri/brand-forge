...
- `defaultSemanticMapping` — 7 seeds → **70 semantic tokens**, plus the stacking order and the app frame, every text and border colour chosen
  ...
  washes whose alphas are solved against the palette, so one of each works on every surface, and the
  elevation ladder is five named levels with shadows paired by name.
